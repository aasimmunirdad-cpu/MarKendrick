    ...
    <div data-testid="work-page" className="pt-32 sm:pt-40 pb-24">
      ...
      <div className="max-w-[1400px] mx-auto px-5 sm:px-8">
        <Reveal className="mb-16 sm:mb-20">
          ...
          <h1 className="font-display font-extrabold tracking-tighter text-5xl sm:text-7xl mb-6">Proof, not<br /><span className="text-vermilion">promises.</span></h1>
              ...
              <Reveal key={cs.slug}>
                <Link to={`/work/${cs.slug}`} data-testid={`work-card-${cs.slug}`} className="group grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                  ...
                  <div className={`lg:col-span-5 ${i % 2 === 1 ? "lg:order-1" : ""}`}>
                    ...
                    <h2 className="font-display text-2xl sm:text-3xl font-bold tracking-tighter mb-4 group-hover:text-vermilion transition-colors">{cs.title}</h2>
                    <p className="text-muted-foreground leading-relaxed mb-6">{cs.summary}</p>
                    <div className="flex flex-wrap gap-8">
                      {cs.results.map((r) => (
                        <div key={r.label}>
                          <p className="font-display text-3xl font-extrabold text-vermilion tracking-tighter">{r.metric}</p>
